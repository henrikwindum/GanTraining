...
# We implement the following basic neural network layers based on the code provided in https://github.com/microsoft/StyleSwin/tree/main/models.
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style):
        ...
        style = self.modulation(style).view(style).view(batch, 1, in_channel, 1, 1)
...
class SinusoidalPositionalEmbedding(nn.Module):
    def __init__(self, embedding_dim, padding_idx, init_size=64, div_half_dim=False, center_shift=None):
        super().__init__()
    ...
    def get_embedding(num_embeddings, embedding_dim, padding_idx=None, div_half_dim=False):
        """Build sinusoiodal embeddings

        This matches the implementation in tensor2tensor, but differes slightly
    ...
    def make_grid2d(self, height, width, num_batches=1, center_shift=None):
        ...
        # Note that the index started from 1 since zero will be padding idx 
        # axis -- (b, h or w)
        ...
        # Note that repeat will copy data. If use learned emb, expand my be better
    ...
    def make_grid2d_like(self, x, center_shift=None):
        ...
        Note that the positional embedding highly depends on the function,
        ''make_positions''.
        """
        h, w = x.shape[-2:]
